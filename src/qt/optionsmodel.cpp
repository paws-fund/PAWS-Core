...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePAWSAmount"))
        settings.setValue("nAnonymizePAWSAmount", 1000);

    nAnonymizePAWSAmount = settings.value("nAnonymizePAWSAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePAWSAmount"))
        SoftSetArg("-anonymizepawsamount", settings.value("nAnonymizePAWSAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePAWSAmount:
            return QVariant(nAnonymizePAWSAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePAWSAmount:
            nAnonymizePAWSAmount = value.toInt();
            settings.setValue("nAnonymizePAWSAmount", nAnonymizePAWSAmount);
            emit anonymizePAWSAmountChanged(nAnonymizePAWSAmount);
            break;
        case CoinControlFeatures:
